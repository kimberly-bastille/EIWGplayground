...
summarize_me_for_fun<- function(data, temporal = "annual"){
  ...
  if(temporal == "monthly"){
    ...
    dat_all<- read.csv(here::here("data-eiwg", paste0(i, ".csv"))) %>% 
      ...
      dplyr::mutate(Mon = stringr::str_extract(Date, pattern = "[A-Z]+[a-z]+"),
                    Mon = dplyr::case_when(Mon == "Jan" ~ 1, 
                                           Mon == "Feb" ~ 2, 
                                           Mon == "Mar" ~ 3,
                                           Mon == "Apr" ~ 4,
                                           Mon == "May" ~ 5,
                                           ...
                                           Mon == "Nov" ~ 11,
                                           Mon == "Dec" ~ 12),
                    year = as.numeric(stringr::str_extract(Date, pattern = "\\d+")), 
                    new_date = 1:length(year)) %>% 
  ...
  sum_dat <- dat_all  %>% 
    ...
    dplyr::left_join(dat_all2, by = c("Date", "Mon", "year", "new_date", "value")) %>% 
  ...
  summary_df<- sum_dat %>% 
    dplyr::mutate(delta = stats::predict(lm(new_date~value))[length(stats::predict(lm(new_date~value)))],
                  stats::predict(lm(year~value))[1], 
                  Z = abs(delta)-(sd(value, na.rm=TRUE)), 
                  trend = ifelse(Z > 0, paste0("significantly"), paste0("")), 
                  gauge = round(stats::ecdf(value)(mn5) *100)) %>% 
    
    #dplyr::left_join(dat_all2, by = c("region", "title" ,"y_lab")) %>% 
    dplyr::summarise(indicator = unique(i), 
                     #region = unique(region), 
                     #year = unique(year),
                     mn = unique(mn), 
                     mn5 = unique(mn5),
...
for (i in datalist) {
  dat<-summarize_me_for_fun(i, temporal = "annual")
...
for (i in datalist) {
  dat<-summarize_me_for_fun(i, temporal = "monthly")
  summarydf<-summarydf %>% rbind(dat)
}
